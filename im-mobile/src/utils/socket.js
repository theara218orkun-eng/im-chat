...
export const createSocketConnection = () => {
  ...
  const { wsUrl } = config[process.env.NODE_ENV] || config.development
  ...
  return new Promise((resolve, reject) => {
    ...
    socketTask.onMessage((res) => {
      try {
        ...
        messageStore.handleSocketMessage(data)
      } catch (e) {
    ...
    socketTask.onClose((res) => {
      ...
      clearHeartbeat()
      if (res.code !== 1000) { // Normal closure
        scheduleReconnect()
      }
    })
...
// Export socket store for Vue components
